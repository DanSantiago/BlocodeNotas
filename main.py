...
sg.ChangeLookAndFeel("LightGray1")  # Mudança do Tema
...
menu_layout = (
    ...
    ["Editar", ["Tornar caixa alta", "Tornar caixa baixa", "Titularizar", "Capitalizar"]]
)
...
def tornar_titulo():
    window["_BODY_"].update(value=str(values["_BODY_"]).title())

def capitalizar():
    window["_BODY_"].update(value=str(values["_BODY_"]).capitalize())
...
while True:
    ...
    if event == "Tornar caixa baixa":
        tornar_caixa_baixa()
    if event == "Capitalizar":
        capitalizar()
    if event == "Titularizar":
        tornar_titulo()
